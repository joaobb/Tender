...
const handleLike = async (req, res, next) => {
	try {
		...
		if (!recipeID || recipeID.trim() === '') throw new ReqError('Please send the recipe ID you want to like.', 400);
...
const handlePass = async (req, res, next) => {
	try {
		const recipeID = req.params.recipeID;
		if (!recipeID || recipeID.trim() === '') throw new ReqError('Please send the recipe ID you want to pass on.', 400);
