...
const Tender = () => {
	...
	useEffect(() => {
		const fetchRecipes = async () => {
			...
			const statedRecipes = response.data.reduce((acc, recipe) => {
				if (!recipes.some((rec) => recipe._id === rec._id)) acc = [...acc, { ...recipe, state: RECIPE_GOT.WAITING }];
				return acc;
			}, []);

			if (statedRecipes.length) setRecipes((curr) => [...curr, ...statedRecipes]);
		};
	...
	const handleSwipe = async (isLike) => {
		...
		Notificate(`Congratulations, you've matched with ${recipe.name}.`, 'success', 'top-center', true);
	...
	const handleAction = (event) => {
		...
		if (recipes.length) setRecipes([{ ...recipes[0], state: newState }, ...recipes.slice(1)]);
	};
...
const Card = ({
	...
	const setRotation = ({ x = 0, y = 0, deg = 0 }) => {
		if (card.current) {
			...
			card.current.style.transformOrigin = `bottom ${deg > 1 ? 'right' : 'left'}`;
		}
	};
	...
	const handlePress = (event) => {
		...
		const x = OFFSET_X - (event.touches ? event.touches[0].clientX : event.clientX);
		const y = OFFSET_Y - (event.touches ? event.touches[0].clientY : event.clientY);
	...
	const handleUnpress = (event) => {
		...
		const x = OFFSET_X - (event.changedTouches ? event.changedTouches[0].clientX : event.clientX);
		const y = OFFSET_Y - (event.changedTouches ? event.changedTouches[0].clientY : event.clientY);
	...
	useEffect(() => {
		if (state !== 0) {
			handleSwipe(state === RECIPE_GOT.LIKED);
	...
	return (
		...
		>
			<RecipeCard image={image} title={title} tags={tags} serves={serves} prepTime={prepTime} />
		</Drag>
	);
};
