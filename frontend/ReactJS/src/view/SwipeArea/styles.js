...
export const Drag = styled.div`
	...
	bottom: ${({ position }) => position * 10 + 100}px;
	animation: ${({ isActive }) => (isActive ? cardChange : 'none')} 200ms cubic-bezier(0, 0.35, 0.7, 1.1);

	filter: blur(${({ position }) => position}px);
	...
	transition: ${({ hasTransition }) => (hasTransition ? 'all 300ms ease' : 'none')};

	&.complete-swipe--right {
		transform-origin: bottom right !important;
		animation: ${completeSwipeRight} 500ms cubic-bezier(0, 0.35, 0.7, 1.1) forwards;
	}
	&.complete-swipe--left {
		transform-origin: bottom left !important;
		animation: ${completeSwipeLeft} 500ms cubic-bezier(0, 0.35, 0.7, 1.1) forwards;
	}
	...
	${({ isActive, initialRotation }) => !isActive && `transform: rotate(${initialRotation}deg)`}
`;
...
export const LikeButton = styled(ActionButton)`
	...
	&:active {
		animation: ${wok_move} 800ms cubic-bezier(0, 0.35, 0.7, 1.1);
	}
